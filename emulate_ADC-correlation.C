/**********************************************************************
 ...
 Purpose    : Emulate the ADC correlation plot for a given channel (This only has been tested for BC runs)
...
int main(int argc, char** argv)
{
  ...
  if(argc >= 6){
    std::istringstream isRefTCNum(argv[5]);
    isRefTCNum >> refTC;
  }
  
  if(argc >= 7){
    std::istringstream isRefTcChNum(argv[6]);
    isRefTcChNum >> refTcCh;
  }
  ...
  for(int ieloop=0;ieloop<nloop;ieloop++){
    ...
    for(const auto& event : eventList){
      ...
      //if(!isTcCh(econtarray.at(event), hrocarray.at(event), refTC, refTcCh)) continue;
