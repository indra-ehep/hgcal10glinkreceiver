...
namespace Hgcal10gLinkReceiver {
  ...
  ProcessorBase() : _printEnable(false), _checkEnable(false), _assertEnable(false) {
    ...
    virtual bool configuring() {
      return true;
    }
    
    virtual bool reconfiguring() {
      return true;
    }
	...
	else if(_ptrFsmInterface->systemState()==FsmState::Configuring  ) this->configuring();
	else if(_ptrFsmInterface->systemState()==FsmState::Reconfiguring) this->reconfiguring();
	...
	else if(_ptrFsmInterface->systemState()==FsmState::Halting      ) this->halting();
	...
	else if(_ptrFsmInterface->systemState()==FsmState::Configured ) this->configured();
